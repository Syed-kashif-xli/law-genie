// var hostIP = "https://app.ecourts.gov.in/ecourt_mobile_encrypted_DC/";
var hostIP = "https://app.ecourts.gov.in/ecourt_mobile_DC/";
// var hostIP = "http://10.249.33.50/ecourt_mobile_DC/";  
// var hostIP = "http://10.153.16.219/ecourt_mobile_encrypted/ecourt_mobile_DC/";//jwt live setup
// var hostIP = "http://10.153.16.219/ecourt_mobile_encrypted/ecourt_mobile_encrypted_DC/";//jwt implementation
//var hostIP = "http://10.153.6.215/ecourt_mobile_encrypted_audit/ecourt_mobile_DC/";
// var hostIP = "http://10.153.181.14/ecourt_mobile_encrypted/ecourt_mobile_DC/"; 
...
function resetLabelsOnIndexPage(obj){
    
    window.globalServerLabelsJSONObj = obj;                    
    window.sessionStorage.setItem("GLOBAL_LABELS", JSON.stringify(globalServerLabelsJSONObj));
    courtComplexSelectLabel = globalServerLabelsJSONObj[268];
    
    labelsarr = window.sessionStorage.GLOBAL_LABELS ? JSON.parse(window.sessionStorage.GLOBAL_LABELS) : null;
    
    totalNoOfEstLabel = labelsarr ? labelsarr[390] : "Total Number of Establishments in Court Complex";
    totalNoOfCasesLabel = labelsarr ? labelsarr[83] : "Total Number of Cases";
    ...
    $("#txt_cnr_tab").text(labelsarr[144]);
    $("#txt_case_status_tab").text(labelsarr[11]);
    $("#txt_causelist_tab").text(labelsarr[190]);   
    $("#txt_mycases_tab").text(labelsarr[635]);   

    stateSelectLabel = labelsarr[106];
    districtSelectLabel = labelsarr[107];    
    $( "#state_code option[value='']" ).text(stateSelectLabel);
    $( "#dist_code option[value='']" ).text(districtSelectLabel);

    $("#searchCNRId").attr("placeholder",labelsarr[451]);
    $("#td_OR").html(labelsarr[736]);
    $("#scan_txt").html(labelsarr[737]);
    $("#titleText").html(labelsarr[738]);
    $("#txtDisclaimer").html(labelsarr[110]);
    $("#howtoSpan").html(labelsarr[488]);
    $("#howtoSpan_a").html(labelsarr[752]);//TODO
    $("#howtoSpan_b").html(labelsarr[740]);
    $("#howtoSpan_c").html(labelsarr[741]);
    $("#howtoSpan_d").html(labelsarr[742]);
    $("#howtoSpan_e").html(labelsarr[743]);
    $("#howtoSpan_f").html(labelsarr[744]);
    $("#howtoSpan_g").html(labelsarr[745]);
    setTimeout(function () { 
        $("#versions").html(labelsarr[802]+": " + versionstr);
        var appReleaseStr=sessionStorage.getItem("appReleaseStr");                
        if(appReleaseStr==null || appReleaseStr=="" || appReleaseStr.toString()=="null"){
        }else{  
            $("#newVersionAvailabel").html(labelsarr[800]+" " + appReleaseStr + " "+labelsarr[801]);
            $("#newVersionAvailableId").html(labelsarr[800]+" " + appReleaseStr + " "+labelsarr[801]);
        }
...
function populateLabelsRadioButtons(languages){
    if(languages != null){
        for(var i = 0 ; i < languages.length ; i++){
            var languagesObj = languages[i];
            var radioBtn = $('<label class='+languagesObj.id+'_label><input type=radio id='+languagesObj.id+' name=lang value='+languagesObj.value+'/>'+languagesObj.value + '</label>');
            radioBtn.appendTo('#lang_selected');            
        }
        $("."+localStorage.getItem("LANGUAGE_FLAG")+"_label").addClass("active_court_label");
        var $radios = $('input[name=lang]');
        $radios.filter('[id='+localStorage.getItem("LANGUAGE_FLAG")+']').prop('checked', true); 
    }
}


function resetAllTabPanels(){
    $("#court_code option[value='']").text(courtComplexSelectLabel);
    $("#Case_Status_pannel").trigger("languageChanged");
    $("#Causelist_pannel").trigger("languageChanged");
    $("#My_Cases_pannel").trigger("languageChanged");
    $("#header_simple_sidebar").trigger("languageChanged");
}
